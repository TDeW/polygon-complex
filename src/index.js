...
export default feature => {
  ...
  const selfIsectsData = selfIntersections(feature, (...args) => args);
  ...
  for (let j = 0; j < ringCount; j++) {
    ...
    const windingAtIsect = isConvex([isectList[isectBeforeLeftIsect].coord, isectList[leftIsect].coord, isectList[isectAfterLeftIsect].coord], true) ? 1 : -1;
...
function isConvex (pts, righthanded) {
  ...
  const d = (pts[1][0] - pts[0][0]) * (pts[2][1] - pts[0][1]) - (pts[1][1] - pts[0][1]) * (pts[2][0] - pts[0][0]);
